...
export class KeycloakHelper {
     ...
     */
    async createUsersAndGroups(realm, options) {
        await this._ensureAdminClient();
     ...
     */
    async getGroupsOfUser(realm, username) {
        await this._ensureAdminClient();
        this._adminClient.setConfig({ realmName: realm });
    ...
    async deleteUser(realm, username) {
        if (DEFAULT_USERS.some((u) => u.username === username)) {
            throw new Error(`Deleting default Keycloak user "${username}" is not permitted.`);
        }
        await this._ensureAdminClient();
        this._adminClient.setConfig({ realmName: realm });
        const users = await this._adminClient.users.find({ username });
    ...
    async deleteGroup(realm, groupName) {
        if (DEFAULT_GROUPS.some((g) => g.name === groupName)) {
            throw new Error(`Deleting default Keycloak group "${groupName}" is not permitted.`);
        }
        await this._ensureAdminClient();
        this._adminClient.setConfig({ realmName: realm });
     ...
     */
    async deleteUsersAndGroups(realm, options) {
        await this._ensureAdminClient();
        const { groups = [], users = [] } = options;
