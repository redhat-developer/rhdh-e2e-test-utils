...
import { generatePluginsFromMetadata, processPluginsForDeployment, getNormalizedPluginMergeKey, disablePluginWrappers, } from "../../utils/plugin-metadata.js";
...
export class RHDHDeployment {
     ...
     */
    async _mergeGeneratedWithBase(generatedConfig) {
    ...
    async _buildDynamicPluginsConfig() {
        ...
        if (userConfigExists) {
            ...
            this._log(`No user config at '${userConfigPath}', auto-generating from metadata...`);
            const generated = await generatePluginsFromMetadata(WorkspacePaths.metadataDir);
    ...
    async _deployWithHelm(valueFile) {
        ...
        const valueFilePath = path.join(os.tmpdir(), `${this.deploymentConfig.namespace}-value-file.yaml`);
    ...
    async _deployWithOperator(subscription) {
        ...
        if (catalogIndexImage) {
            ...
            const extraEnvs = (app.extraEnvs ??=
                {});
            const envs = (extraEnvs.envs ??=
                []);
            envs.push({
