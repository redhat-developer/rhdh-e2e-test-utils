...
`rhdh.deploy()` uses `runOnce()` internally — its key carries the namespace, so it is already once per project, even across worker restarts. No wrapping needed unless there's other expensive setup: use `test.runOnce("key", fn, { scope: "project" })` for that, because `runOnce` defaults to once per *run* and one spec matched by two projects would otherwise leave the second with no setup.
