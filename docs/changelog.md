...
- **`runOnce` accepts a scope**: `test.runOnce(key, fn, { scope: "project" })` keys the flag file per Playwright project, so setup that belongs to one project runs once for each. The default is unchanged (`"run"`, once for the whole test run) because both intents are real and the API cannot guess: installing an operator into a fixed namespace that every project then uses wants once per run, while anything touching a project's own namespace wants once per project. Callers outside a Playwright context can pass `project` explicitly; a `{ scope: "project" }` call that cannot see a project **throws** rather than quietly falling back to once per run, since falling back is the shared-key bug again. Every documented example that wraps `configure()` and `deploy()` — the guide, the API reference, and the overlay tutorials — now passes the option, because those are per-project by nature.

### Fixed

- **`runOnce` silently skipped a second project's setup** — *upgrading alone does not fix an affected call site; it has to opt in.* The flag file was keyed by the key string alone, inside a directory keyed only on the runner PID and therefore shared by every project in the run. When one spec was matched by more than one project — which is what adding an `-app-next` lane does — the first project's setup satisfied the second, and the second skipped `configure()` and `deploy()` entirely, then failed much later on a missing element with nothing pointing at the cause. Seen on `rhdh-plugin-export-overlays#3318`. The default is deliberately unchanged, so what 2.1.10 adds is the fix (`{ scope: "project" }`) and a way to find the call sites that need it: keys now record which project satisfied them, and a key with **no declared scope** that is skipped on behalf of a *different* project logs a warning naming both. An explicit `{ scope: "run" }` is left alone — sharing one key is a real intent and the author has already said so — and a skip within the same project, the worker-restart case the helper exists for, stays quiet.
