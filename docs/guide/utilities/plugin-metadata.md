...
- **DPDY + OCI**: Use `{{inherit}}` tag — RHDH resolves the version from its built-in DPDY. This tests the exact versions shipped in the RC. No config injection (RHDH provides defaults). The registry defaults to `registry.access.redhat.com/rhdh` and can be overridden with `NIGHTLY_DPDY_OCI_REGISTRY` (blanket) or `NIGHTLY_DPDY_OCI_REGISTRY_MAP` (per-plugin JSON, takes precedence).
